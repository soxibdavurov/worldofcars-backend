...
export class MemberService {
	...
	public async login(input: LoginInput): Promise<Member> {
		...
		const nickNormalized = (memberNick ?? '').trim().toLowerCase();
		const memberNickRegex = new RegExp(`^${(nickNormalized || '').replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}$`, 'i');
		const response = await this.memberModel.findOne({ memberNick: memberNickRegex }).select('+memberPassword').exec();
	...
	public async getMember(memberId: ObjectId | null, targetId: ObjectId): Promise<Member> {
		...
		// Set default value for memberCars if not exists (for old members)
		if (targetMember.memberCars === undefined || targetMember.memberCars === null) {
			targetMember.memberCars = 0;
		}
